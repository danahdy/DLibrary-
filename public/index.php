...
function markTokenAsUsed($token) {
    ...
    try {
        ...
        $deleteSql = "DELETE FROM tokens WHERE token = :token";
        $deleteStmt = $conn->prepare($deleteSql);
        $deleteStmt->bindParam(':token', $token);
        $deleteStmt->execute();

    } catch (PDOException $e) {
        error_log("Error in markTokenAsUsed: " . $e->getMessage());
    }
}
...
$app->post('/book_author/register', function (Request $request, Response $response, array $args) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT COUNT(*) FROM book_authors WHERE bookid = :bookid AND authorid = :authorid");
...
$app->get('/book_author/show', function (Request $request, Response $response) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT collectionid, bookid, authorid FROM book_authors");
        $stmt->execute();
